...
var states = {
	...
	"request": function (req, res) {
		...
		if (spServiceConf){
            ...
            if (spServiceConf.request) {
                if (spServiceConf.request.headers)
                    requestParams.headers = spServiceConf.headers;
                if (spServiceConf.request.json)
                    requestParams.json = spServiceConf.request.json;
			}
		}
		...
		requestParams.url = restUrl + urlToCall;
		...
		req.soajs.model.request(req, res, mergedParams, function (error, response, body) {
			if (error) {
			...
			//TODO: add response mapping here
