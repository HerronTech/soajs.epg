...
function getMySchema(req) {
	...
	return req.soajs.config.schema.post[schema];
}
...
var states = {
	...
	"find": function (req, res, next) {
		...
		let originalParams = {
			url: "http://dashboard-api.soajs.org/urac/account/getUser",
			method: "get",
			body,
			qs,
			headers
		...
		let mergedParams = merge2Params(req.soajs.authAddOnsParams, originalParams); // beyond auth variable
		
		req.soajs.model.request(mergedParams, function (error, response, body) {
			if (error) {
				var error = {
					code: 405,
