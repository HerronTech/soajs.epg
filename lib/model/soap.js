...
function request(soajs, params, cb) {
	...
	// clean headers
	delete deliveryOptions.headers["host"];
	delete deliveryOptions.headers["soajsinjectobj"];
	delete deliveryOptions.headers["content-length"];
	delete deliveryOptions.headers["user-agent"];
	delete deliveryOptions.headers["x-nginx-proxy"];
	delete deliveryOptions.headers["x-forwarded-proto"];
	delete deliveryOptions.headers["x-forwarded-for"];
	
	soajs.log.debug('---- start deliveryOptions -----');
	soajs.log.debug(JSON.stringify(deliveryOptions, null, 2));
	soajs.log.debug('---- end deliveryOptions -----');
	...
	nodejsrequest[params.method.toLowerCase()](deliveryOptions, function (error, response, body) {
		...
		try {
			soajs.log.debug('---- start body -----');
			soajs.log.debug(body);
			soajs.log.debug('---- end body -----');
