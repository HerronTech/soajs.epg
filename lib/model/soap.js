 ...
 * @param cb
 */
function model(soajs, params, cb) {
	let options = {};
		...
		if (err) {
			...
			if (client) {
				...
				if(Object.hasOwnProperty.call(client, operation) ){
					...
					var xmlBody = constructXML(params.body);
					...
					request[params.method.toLowerCase()](deliveryOptions, function (error, response, body) {
						if (error) {
							return cb(error);
						}
